...
class Article
{
    ...
    {
        $stmt = $this->pdo->prepare('INSERT INTO articles (title, content, category, status, author_id, thumbnail, created_at, updated_at) VALUES (:title, :content, :category, :status, :author_id, :thumbnail, NOW(), NOW())');
        $stmt->execute([
            ':title' => $data['title'],
    ...
    {
        $stmt = $this->pdo->prepare('UPDATE articles SET title = :title, content = :content, category = :category, status = :status, thumbnail = :thumbnail, updated_at = NOW() WHERE id = :id');
        return $stmt->execute([
            ':id' => $id,
            ':title' => $data['title'],
            ':content' => $data['content'],
